...
struct Parallelogram : Shape {
    ...
    Parallelogram(Point tl, Point tr, Point bl) : br_x(tl.x+bl.x-tr.x), br_y(tl.y+bl.y-tr.y) //: br(tl+bl-tr)
    {
        add(tl);
        add(tr);
	add(bl);
	Point br = Point(br_x,br_y);
	//add(Point(br_x,br_y));
	add(br);
    }
	void draw_lines() const;

    int bott_right_x() const { return br_x; }
    int bott_right_y() const { return br_y; }
    // int bott_right() const (return br; )
private:
    int br_x;    // bottom right point of parallelogram
    int br_y;
    // int br;
};
